...
ic50data_all=read.csv("data/ic50data/IC50HeatMap_v2.csv",header = T,stringsAsFactors = F)
unique(ic50data_all$species)%in%(c("E255K","E255V","E255I"))
...
ic50data_all=ic50data_all%>%filter(species%in%c("D276G",
                                                "E255K",
                                                "E255V",
                                                "E355A",
                                                "E355G",
...
for(species_curr in sort(unique(ic50data_long$mutant))){
  ...
  for(conc.curr in conc.list){
    ...
    ic50.model.pred.species.rep.specific$ic50[i]=e
    i=i+1
  }
...
Getting the mean +- sd of the ic50 of the fits across species by each technical replicate

```{r}

ic50s_summarized=ic50.model.pred%>%group_by(mutant)%>%
  summarise(ic50_u=round(mean(ic50)*1000,1),ic50_sd=round(sd(ic50)*1000,1))
...
# Decide on color values for plot
...
# class(ic50data_long$species)


label_positions <- ic50data_long %>%
  filter(!species%in%"M244V")%>%
  group_by(species) %>%
  summarise(
    x = 10,             # consistent left edge on log10 scale
    y = 0.15,           # high enough for all panels
    label = unique(species)
  )


ggplot(ic50data_long%>%filter(!species%in%"M244V"),aes(x=dose*1000,y=rel.viab))+
  ...
  scale_y_continuous("Relative viability",
                     limits=c(0,1.2),breaks = c(0,0.5,1))+
...
ggsave("data/ic50data/mutantstandards_v2.pdf",width=6,height=7,useDingbats=F)
