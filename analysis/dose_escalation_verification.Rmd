...
### Downloading mutant standards data
```{r}
...
# 
# ggplot(ic50data_long,aes(x=conc,y=y_model))+
...
### Downloading mutant verification data (Pooled mutant DMS data)
```{r}
...
ggsave("data/IC50s_verification/IC50s_clonalvsdms.pdf",width=7,height=4,useDingbats=F)
...
data_dms_long <- data_dms %>%
  ...
  mutate(Dose = as.numeric(Dose))  # Convert Dose to numeric for easier analysis
# data_dms_long2=data_dms_long
data_dms_long=data_dms_long%>%filter(Dose%in%c(seq(10,90,10),
                                               seq(110,1010,100),
                                               seq(1020,9820,500)))
# data_dms_long=data_dms_long%>%mutate(Dose=Dose/1000)
# Data_dms_simple
...
# data_dms_long_1250=data_dms_long%>%filter(Dose==1470)
species_resistance <- data_dms_long %>%
  filter(Dose == 7020) %>%
  group_by(species) %>%
  summarise(mean_viab = mean(rel.viab, na.rm = TRUE)) %>%
  arrange(desc(mean_viab)) %>%
  mutate(species = factor(species, levels = species))  # most resistant = highest viability
data_dms_long <- data_dms_long %>%
  mutate(species = factor(species, 
                          levels = species_resistance$species))
# data_dms_long$species=factor(data_dms_long$species,levels = as.character(data_dms_long_1250$species[order((data_dms_long_1250$rel.viab),decreasing = T)]))
data_dms_long=resmuts_adder(data_dms_long)
data_dms_long <- data_dms_long %>%
  arrange(resmuts, Dose)

ggplot(data_dms_long,aes(x=Dose,y=rel.viab,fill=species,color=resmuts))+
  geom_line()+
  scale_x_continuous(trans="log10")+
  cleanup+
  theme(legend.position="none")+
...
ggplot(data_dms_long, aes(x = Dose, y = rel.viab, group = species, color = resmuts)) +
  geom_line(alpha = 0.4, linewidth = 0.6) +
  scale_x_log10(name = "Dose (log10 scale)") +
  scale_y_continuous(name = "Relative Viability", limits = c(0, 1)) +
...
ggplot() +
  # 1. Plot non-resistant mutants (gray, in the background)
  geom_line(
    data = subset(data_dms_long, resmuts == FALSE),
    aes(x = Dose, y = rel.viab, group = species),
    color = "gray85", alpha = 0.5, linewidth = 0.5
  ) +
  # 2. Plot resistant mutants (red, on top)
  geom_line(
    data = subset(data_dms_long, resmuts == TRUE),
    aes(x = Dose, y = rel.viab, group = species),
    color = "red", alpha = 0.9, linewidth = 0.8
  ) +
  scale_x_log10(name = "Dose (log10 scale)") +
  scale_y_continuous(name = "Relative Viability", limits = c(0, 1)) +
  theme_minimal(base_size = 14) +
...
ggsave("5000ic50sv2.pdf",width = 5,height = 4,units = "in",useDingbats=F)


data_dms_long <- data_dms_long %>%
  mutate(
    resmuts2 = case_when(
      species %in% c("A337V","P465S","V468F","I502L","V299L") ~ "sensitive",
      resmuts == TRUE ~ "resistant",
      TRUE ~ "nonresistant"
    )
  )

ggplot(data_dms_long, aes(x = Dose, y = rel.viab, group = species)) +
  geom_line(aes(color = resmuts2), alpha = 0.6, linewidth = 0.6) +
  scale_color_manual(
    values = c(
      "nonresistant" = "gray85",
      "resistant" = "red",
...
ggplot() +
  # 1. Nonresistant (gray, background)
  geom_line(
    data = filter(data_dms_long, resmuts2 == "nonresistant"),
    aes(x = Dose, y = rel.viab, group = species),
    color = "gray85", alpha = 0.5, linewidth = 0.5
  ) +
  # 2. Resistant (red, middle)
  geom_line(
    data = filter(data_dms_long, resmuts2 == "resistant"),
    aes(x = Dose, y = rel.viab, group = species),
    color = "red", alpha = 0.9, linewidth = 0.8
  ) +
  # 3. Sensitive (blue, on top)
  geom_line(
    data = filter(data_dms_long, resmuts2 == "sensitive"),
    aes(x = Dose, y = rel.viab, group = species),
    color = "blue", alpha = 0.9, linewidth = 0.8
  ) +
  scale_x_log10(name = "Dose (log10 scale)") +
  scale_y_continuous(name = "Relative Viability", limits = c(0, 1)) +
  theme_minimal(base_size = 14) +
...
ggplot(data_dms_long, aes(x = Dose, y = rel.viab)) +
  stat_density_2d(aes(fill = ..level..), geom = "polygon", contour = TRUE, alpha = 0.6) +
  scale_fill_viridis_c(option = "C") +  # or use "plasma", "inferno", etc.
  # geom_line(aes(group = species), color = "gray30", alpha = 0.3, linewidth = 0.4) +
  scale_x_log10(name = "Dose (log10 scale)") +
  scale_y_continuous(name = "Relative Viability", limits = c(0, 1)) +
  theme_minimal(base_size = 14) +
...
auc_data <- data_dms_long %>%
  arrange(species, Dose) %>%
  group_by(species) %>%
  summarise(
    auc = trapz(Dose, rel.viab),
    .groups = "drop"
  )

auc_data <- auc_data %>%
  mutate(
    auc_quintile = ntile(auc, 5)  # creates values 1–5
  )

data_with_quintile <- data_dms_long %>%
  left_join(auc_data, by = "species")


ggplot(data_with_quintile, aes(x = Dose, y = rel.viab, group = species, color = factor(auc_quintile))) +
  geom_line(alpha = 0.6, linewidth = 0.6) +
  scale_x_log10(name = "Dose (nM)") +
  scale_y_continuous(name = "Relative Viability", limits = c(0, 1)) +
  scale_color_brewer(palette = "RdYlBu", direction = -1, name = "Resistance\n Quintile") +
  theme_minimal(base_size = 14) +
  theme(
    panel.grid.minor = element_blank(),
...
ggsave("5000ic50s.pdf",width = 5,height = 4,units = "in",useDingbats=F)



data_dms_long <- data_dms_long %>%
  # left_join(select(your_ic50_table, species, IC50), by = "species") %>%
  mutate(
    ic50_bin = case_when(
      IC50 < 300 ~ "<300",
      IC50 < 600 ~ "300–600",
...
data_dms_long$ic50_bin=factor(data_dms_long$ic50_bin,
                              levels=c(">1200",
                                       "600–1200",
                                       "300–600",
                                       "<300",
...
ggplot(data_dms_long%>%filter(!ic50_bin%in%NA), aes(x = Dose, y = rel.viab, group = species, color = ic50_bin)) +
  geom_line(alpha = 0.6, linewidth = 0.6) +
  scale_x_log10(name = "Dose (nM)") +
  scale_y_continuous(name = "Relative Viability", limits = c(0, 1)) +
  scale_color_manual(
...
ggsave("5000ic50sv3.pdf",width = 5,height = 4,units = "in",useDingbats=F)
```
