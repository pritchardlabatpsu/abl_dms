...
```{r}
# rm(list=ls())
gnomad=read.csv("data/gnomad_ABL/gnomAD_v4.1.0_ENSG00000097007_2025_07_27_16_14_35.csv")
...
```
## Looking at how imatinib resistant gnomad-enabled mutants are
### i.e. merging with the DMS data
...
# resistance_predictions=read.csv("output/ABLEnrichmentScreens/ABL_Region1234_Tileseq/Tileseq_clinical_predictions_9.12.25.csv",header = T,stringsAsFactors = F)
# resistance_predictions=read.csv("output/ABLEnrichmentScreens/ABL_Region1234_Tileseq/Tileseq_clinical_predictions_9.19.25.csv",header = T,stringsAsFactors = F)
# resistance_predictions=read.csv("output/ABLEnrichmentScreens/ABL_Region1234_Tileseq/Tileseq_clinical_predictions_10.16.25.csv",header = T,stringsAsFactors = F)
resistance_predictions=read.csv("output/ABLEnrichmentScreens/ABL_Region1234_Tileseq/Tileseq_clinical_predictions_10.28.25.csv",header = T,stringsAsFactors = F)
...
```
## Generating supplemental table s7
```{r}
...
write.csv(tables7,"data/gnomad_abl/table_s9.csv")
```
```{r}
# a=imatdata_gnomad%>%filter(!fill_value%in%"Sensitive")
...
# ggsave("output/gnomad_figures/gnomad_enabled_mutants.pdf",width=12,height = 4, units="in",useDingbats=F)
...
ggplot(imatdata_gnomad%>%arrange(desc(rank)),aes(x=species,y=netgr_obs_mean_corr,label=species))+
  ...
  theme(legend.position = c(0.1,.2),
        ...
        axis.ticks.x = element_blank(),
        axis.text.y = element_text(angle = 90, hjust = 0.5, vjust = 0.5))
# ggsave("output/gnomad_figures/gnomad_enabled_mutants_bydose.pdf",width=12,height = 4, units="in",useDingbats=F)
...
# ggsave("output/gnomad_figures/gnomad_enabled_mutants_penetrance2_withlegend.pdf",width=7,height = 4, units="in",useDingbats=F)
...
# ggsave("output/gnomad_figures/gnomad_enabled_mutants_penetrance.pdf",width=6,height = 3, units="in",useDingbats=F)
...
ggplot(imatdata_gnomad%>%filter(dose%in%"1200nM",rank<50)%>%arrange(desc(rank)),aes(x=reorder(species, -netgr_obs_mean_corr),y=netgr_obs_mean_corr,label=species,size=Allele.Frequency.Gnomad))+
  ...
  theme(axis.text.x = element_blank(),
        ...
        axis.title.x = element_text(size=9),
        axis.title.y = element_text(size=9))
ggsave("output/gnomad_figures/gnomad_enabled_mutants_penetrance_zoomed.pdf",width=3.5,height = 2.5, units="in",useDingbats=F)
...
# a=imatdata_gnomad %>%filter(dose%in%"1200nM",!fill_value%in%"Sensitive")
imatdata_gnomad <- imatdata_gnomad %>%
  mutate(species = fct_reorder(species, -rank_alpha444, .desc = TRUE))
ggplot(imatdata_gnomad%>%filter(dose%in%"1200nM",!fill_value%in%"Sensitive")%>%arrange(desc(rank_alpha444)),aes(x=species,y=alpha.444,label=species))+
  # geom_point(color="black",size=2,shape=21,aes(fill=fill_value))+
  geom_col(aes(fill=fill_value),position = position_dodge(1))+
  geom_text(data = netgr_average, aes(x = species, y = ifelse(alpha.444 > 0,
                           alpha.444 + 0.0015, # Move text above positive bars
                           alpha.44 - 0.0015), # Move text below negative bars))+,
                           label = species),
            size = 2, color = "red",
            angle=90)+
  scale_x_discrete("SNP-enabled ABL mutant")+
...
# data <- imatdata_gnomad%>%
#   filter(dose%in%"1200nM") %>% 
#   arrange(desc(netgr_obs_mean_corr)) %>%
#   mutate(prop = netgr_obs_mean_corr / sum(data$netgr_obs_mean_corr) *100) %>%
#   mutate(ypos = cumsum(rank)- 0.5*rank )
...
ggsave("output/gnomad_figures/clin_var_piechart.pdf",width=3.5,height = 2.5, units="in",useDingbats=F)
...
# write.csv(gnomad_enrichment_melt,"data/gnomad_ABL/gnomad_vudrs_byethnicities_melted.csv")
a=gnomad%>%filter(residue%in%c(240:512))
```
...
ggplot(data, aes(x="", y=Number., fill=ancestry)) +
...
# ggsave("output/gnomad_figures/gnomad_piecharts_individuals.pdf",width=2.5,height = 2.5, units="in",useDingbats=F)
#################################################################################
data=gnomad_enrichment_melt%>%
  filter(!Count.%in%0)%>%
  group_by(ancestry)%>%
...
ggplot(data, aes(x="", y=nmuts, fill=ancestry)) +
  ...
  # geom_text(aes(y = ypos, label = round(sum_freq,2)), color = "black", size=4) +
  scale_fill_brewer(palette="Set3")
# ggsave("output/gnomad_figures/gnomad_piecharts_mutants.pdf",width=2.5,height = 2.5, units="in",useDingbats=F)
...
# ggsave("output/gnomad_figures/resistance_distributions.pdf",width=1,height = 3, units="in",useDingbats=F)
...
# ggsave("output/gnomad_figures/gnomad_piechart_overall_percentage.pdf",width=1.5,height = 1.5, units="in",useDingbats=F)
